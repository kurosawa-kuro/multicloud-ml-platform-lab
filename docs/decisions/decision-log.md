...
## 2026-08-02 — 大改修の検証戦略: ローカル証拠で P0〜P7 完了 → 実クラウド検証キャンペーン1回 → 最終修正（owner 採用）

- type: approach-choice
- 根拠 (why): P0/P1/P4〜P7 は等価変形＋番人強化が主体で、`make test`（クラウド資格情報
  なしで走る）と「赤の実演」により EL2〜3 で閉じられる。実クラウドが必須なのは P2/P3 の
  構造変更と D5 フォールバック廃止の確認のみ。クラウドを起こすのが1回で済み月額枠にも
  収まる。8/2 の実験管理・パイプライン一括 EL4 に成功前例がある（ユニット全緑のまま
  実クラウドで欠陥6件を検出→修正して完走）。
- 影響範囲 (blast radius): 中。検証が最後に一点集中するため、実クラウドでしか出ない反例が
  P2 の設計判断にあった場合、手戻りが P2〜P7 に波及しうる。緩和策を制約として固定:
  (a) **コミット粒度を P 単位に保つ**（欠陥原因の bisect 可能性の担保）
  (b) **キャンペーンのチェックリストを実施前に書く**（「何が観測されたら FAIL か」を先に決める
  = evidence-policy）
  (c) P2/P3 は実装完了後 `04_verifying/` で terminal signal（キャンペーン緑）を待つ
  （done にしない）。
- 撤退条件 (stop/revert): キャンペーンで P2 の設計自体を覆す反例が出たら、最終修正で
  粘らず該当 P を rollback して個別検証方式へ切り替える。Azure クレジット失効
  **2026-08-30** までに実施できなければ、キャンペーンを4基盤で先行し Azure は
  01_requirements の「条件付き」判断に従う。
- 結果 (outcome): open（キャンペーン = 大改修P8 task）
- link: [大改修P8](../tasks/02_backlog/2026-08-02-大改修P8-実クラウド検証キャンペーン.md) /
  [調査と方針 第2版](../tasks/01_idea/2026-08-02-大改修-5インフラ制約起点の再設計-調査と方針.md)
