...
class TrackedOperations:
    ...
    継承側でこの層に何かを**足さない**こと。ここに置いてよいのは
    **5基盤に共通する記録の形だけ**（ports.py の「port を切る基準」と同じ判断）。
    単一基盤の関心（例: Vertex AI Experiments への複写）は、その基盤の adapter 内で
    `_tracked` を override して足す —— 2026-08-02 に observer フックをここへ置いて
    その原則を破り、作り直した。
    """
